...
function artworkUpdate(evt) {
	...
	if(/*evt.which == 1*/evt.originalEvent.buttons == 1 || evt.type == 'touchmove') {
...
function artworkDrawOnce(evt) {
	...
	var mousePos = getMousePos(evt);
	if(artworkTool.type < 2) {
	if(/*evt.which == 1*/evt.originalEvent.buttons == 1) {
		if(artworkTool.type == 0) {
			ctx.fillStyle = artworkColor;
		} else {
    	ctx.fillStyle = "#fff";
    ...
    b.Form = {
        ...
        submit: function(b, c) {
            b.trigger("olv:form:submit", [c || a()]);
                var sucky
				if(Olv.fuUUckFiles && Olv.fuUUckFiles.length) {
					var d = new FormData(b[0])
                    /*console.log('screen. screEEEE (window.scee)')
                    window.scee = $('input[type=file]').files*/
                    //d.delete('screen')
					d.append('screen', Olv.fuUUckFiles[0])
					sucky = true
				} else {
        ...
        reset: function(c) {
            //console.log('Olv.Form.Reset called!')
            console.trace()
            c.each(function() {
                this.reset(),
